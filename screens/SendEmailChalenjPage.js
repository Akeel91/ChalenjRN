...
const SendEmailChalenj = ({navigation}) => {
  moment.locale();
  const [showMessage, setShowMessage] = useState(false);
  const [showActionComplete, setShowActionComplete] = useState(false);
  ...
  const contentEmail = apiResp.apiResponse.content;
  ...
  function check() {
    console.log('emailparam--', chalenjId, actionId, actionType);
  }
  function notifyMessage(msg) {
    if (Platform.OS === 'android') {
      ToastAndroid.show(msg, ToastAndroid.SHORT);
          ...
          <View style={{marginVertical: 20}}>
            ...
            <View style={{marginTop: 20}}>
              <ButtonWithIcon
                ...
                btnTitleColor="#fff"
                onPressCallback={() => {
                  Linking.openURL('mailto:' + contentEmail);
